...
def get_words(tei: ET._Element) -> list[dict]: # type: ignore
    ...
    def find_next(synch: str)-> str|None:
        ...
        for i, s in enumerate(all_synchs):
            ...
            raise ValueError(f"Iterated though all synchs, couldn't find a match for {synch}.")
    all_words = tei.findall(".//{*}w")
    return [{**w.attrib, "next_synch": find_next(w.get("synch")), "text": w.text}for w in all_words]
